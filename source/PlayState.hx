...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['see?', 0.6], //From 50% to 59%
		['Amazung', 0.69], //From 60% to 68%
		['Sex Number', 0.7], //69%
		['Amazing', 0.8], //From 70% to 79%
		['Krimah', 0.9], //From 80% to 89%
		['Krimah Fan!', 1], //From 90% to 99%
		['MUCK!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'boy'
					playVideo('mokeycutscene')
					
				case 'ah-hyuck'
					playVideo('groobycutsceneSUS')
			}
			seenCutscene = true;
		} else {
			startCountdown();
